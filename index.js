...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// A: nestedFunction is a closure and the variable is within the lexical context. 
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooAnimals){
    ...
    zooAnimals.forEach((item) => {
      furrbaby.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`)
    });
    return furrbaby;
  }
  ...
  */
  ...
  function lowerCaseNames(zooAnimals){
    let furrbaby = [];
    zooAnimals.map((item) => {
      furrbaby.push(item.animal_name.toLowerCase())
    });
    return furrbaby;
  }
  ...
  */
  ...
  function lowPopulationAnimals(zooAnimals){
    let furrbaby = zooAnimals.filter((item) => {
  ...
  */
  ...
  function USApop(zooAnimals){
    let masses = zooAnimals.reduce((accumulator, item) => {
...
class CubeMaker extends CuboidMakerTwo{
  constructor(cuboidTwo){
    this.equalDim = cuboidTwo.equalDim; 
  }
  speak(){
  return `Volume equals ${this.equalDim * this.equalDim * this.equalDim}, and the Surface area is ${(this.equalDim * this.equalDim) * 6}`;
  }
}
